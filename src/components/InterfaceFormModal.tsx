        ...
        <div className="flex-1 overflow-y-auto p-8">
            ...
            <div className="mb-6">
                 ...
                 <div className="py-2 space-y-5">
                    ...
                    {connectionType === 'DHCP' && (
                        <div className="bg-gray-50 rounded-lg p-6 border border-gray-200 ml-10 relative animate-in fade-in slide-in-from-top-2 duration-200 mt-2">
                            {/* Arrow pointing up */}
                            ...
                            <div className="space-y-5">
                                ...
                                {/* Communication Mode */}
                                <div className="flex items-center">
                                    <label className="w-28 text-sm font-medium text-gray-600 text-right mr-3">通讯方式：</label>
                                    <div className="flex space-x-6">
                                        <label className="flex items-center cursor-pointer">
                                            <input type="radio" checked={dhcpCommunicationMode === 'unicast'} onChange={() => setDhcpCommunicationMode('unicast')} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                                            <span className="ml-2 text-sm text-gray-700">单播</span>
                                        </label>
                                        <label className="flex items-center cursor-pointer">
                                            <input type="radio" checked={dhcpCommunicationMode === 'broadcast'} onChange={() => setDhcpCommunicationMode('broadcast')} className="w-4 h-4 text-blue-600 focus:ring-blue-500" />
                                            <span className="ml-2 text-sm text-gray-700">广播</span>
                                        </label>
                                    </div>
                                </div>
                                {/* System DNS */}
                                <div className="flex items-center">
                                    <label className="w-28 text-sm font-medium text-gray-600 text-right mr-3">作为系统首选DNS：</label>
                                    <label className="flex items-center cursor-pointer">
                                        <input type="checkbox" checked={dhcpSystemDns} onChange={e => setDhcpSystemDns(e.target.checked)} className="w-4 h-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500" />
                                        <span className="ml-2 text-sm text-gray-700">启用</span>
                                    </label>
                                </div>
                            </div>
