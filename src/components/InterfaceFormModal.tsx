...
const InterfaceFormModal: React.FC<InterfaceFormModalProps> = ({ 
  ...
  useEffect(() => {
      if (initialData) {
          ...
          // IP and Gateway can be variables starting with $
          setIpValue(initialData.ip === '-' ? '' : initialData.ip);
          setGatewayValue(initialData.gateway || '');
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="bg-white rounded-lg shadow-xl w-[700px] max-h-[90vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-t border-gray-200 bg-gray-50 flex justify-end items-center space-x-3">
            ...
            <button onClick={handleSave} className="px-8 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded shadow-sm transition-colors flex items-center">{initialData ? '保存修改' : '确定'}</button>
        </div>
      </div>
    </div>
  );
